...
def testDetailJobPage():

    # Start a session   
    driver = webdriver.Chrome(service=ChromeService(executable_path=ChromeDriverManager().install()))

    # Take action on browser
    driver.get("http://localhost:3000/MUNster/jobs/detail-job")
    ...
    apply_button = driver.find_element(by=By.CSS_SELECTOR, value="apply")
    message_button = driver.find_element(by=By.CSS_SELECTOR, value="message")
    job_title = driver.find_element(by=By.ID, value="job-title")
    job_description = driver.find_element(by=By.ID, value="job-description")
    job_details = driver.find_element(by=By.ID, value="job-details")
